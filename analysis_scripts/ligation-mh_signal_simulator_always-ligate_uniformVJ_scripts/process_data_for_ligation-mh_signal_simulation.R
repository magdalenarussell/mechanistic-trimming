...
sim_data = foreach(subset = seq(total/100), .combine=rbind) %dopar% {
    ...
    for (i in seq(100)){
        ...
        # sample a V and J gene with uniform probability
        temp = cbind(gene_choice_probs$v_choice[sample(.N, 1)],
                     gene_choice_probs$j_choice[sample(.N, 1)]) 
...
ANNOTATION_TYPE <<- paste0(old_ANNOTATION_TYPE, '_from_', TRIMMING_PROB_TYPE, '_MHprob', LIGATION_MH_PARAM, '_always-ligate_uniformVJ')
