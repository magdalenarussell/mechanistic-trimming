...
# convert simulated data to no-ligationMH-like annotations
## give all ligation_mh nucleotides to the V-gene (ie. J-gene gets trimmed more)
condensed_sim[, j_trim := j_trim + ligation_mh]

# Adjust trimming sites if necessary based on MH ligation
adjusted_sim = adjust_trimming_sites_for_ligation_mh(condensed_sim[, -c('ligation_mh', 'v_gene_sequence', 'j_gene_sequence')])

# merge in gene sequences
cols = c('v_gene', 'j_gene', 'v_gene_sequence', 'j_gene_sequence')
adjusted_sim = merge(adjusted_sim, unique(condensed_sim[,..cols]), by = c('v_gene', 'j_gene'))

motif_data = get_all_nuc_contexts(adjusted_sim, 'adjusted-mh_sim', gene_type = GENE_NAME, trim_type = TRIM_TYPE)
