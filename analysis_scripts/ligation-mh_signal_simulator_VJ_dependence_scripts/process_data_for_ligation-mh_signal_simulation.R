...
source(paste0(MOD_PROJECT_PATH,'/analysis_scripts/ligation-mh_signal_simulator_VJ_dependence_scripts/ligation-mh_simulator_functions.R'))
...
sim_data = foreach(subset = seq(total/100), .combine=rbind) %dopar% {
    ...
    for (i in seq(100)){
        ...
        v = gene_choice_probs$v_choice[sample(.N, 1, prob = v_gene_prob)] 
        j_choices = gene_choice_probs$j_choice[v_gene == v$v_gene]
        j = j_choices[sample(.N, 1, prob = j_gene_prob)]

        temp = merge(v, j, by = 'v_gene')
...
ANNOTATION_TYPE <<- paste0(old_ANNOTATION_TYPE, '_from_', TRIMMING_PROB_TYPE, '_MHprob', LIGATION_MH_PARAM, '_VJ_dependence')

frame_file_name = str_replace(file_name, old_ANNOTATION_TYPE, ANNOTATION_TYPE)
dir.create(dirname(frame_file_name), recursive = TRUE)
