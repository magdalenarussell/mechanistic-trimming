...
process_file <- function(file){
    ...
    file_data[!(j_gene %like% '\\*') & !(j_gene == ''), j_gene := paste0(j_gene, '*01')]

    # remove -01
    file_data[v_gene %like% '-01', v_gene := str_replace(v_gene, '-01', '')]
    file_data[j_gene %like% '-01', j_gene := str_replace(j_gene, '-01', '')]

    # remove 0
    file_data[v_gene %like% 'V0', v_gene := str_replace(v_gene, 'V0', 'V')]
    file_data[j_gene %like% 'J0', j_gene := str_replace(j_gene, 'J0', 'J')] 
