get_model_feature_scores <- function(validation_data, pwm){
    base_count_pwm = get_all_base_count_pwm_score(validation_data, pwm)
    setnames(base_count_pwm, 'pwm_score', 'base_count_score')
    motif_pwm =  get_all_pwm_score(validation_data, pwm, NULL) 
    setnames(motif_pwm, 'pwm_score', 'motif_score')
    validation_data = merge(validation_data, base_count_pwm, by = c('left_base_count_GC', 'right_base_count_AT', 'right_base_count_GC'))
    positions = get_positions()
    validation_data = merge(validation_data, motif_pwm, by = c('gene', 'trim_length', 'motif', positions))
    return(validation_data)
}

fit_rel_importance_model <- function(motif_data, formula = NULL){
    if (is.null(formula)){
        formula = formula(paste0('cbind(weighted_observation, interaction(gene, subject)) ~ base_count_score + motif_score'))
    }
