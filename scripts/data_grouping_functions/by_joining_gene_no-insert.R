condense_tcr_data <- function(tcr_dataframe){
    cols = c('trimmed_seq', 'sequence', 'gene', paste(TRIM_TYPE), paste0(GENE_NAME), paste(JOINING_INSERT), paste(JOINING_GENE))
    ...
    cols = c('trimmed_seq', 'whole_seq', 'gene', 'trim_length', GENE_NAME, JOINING_INSERT, JOINING_GENE)
