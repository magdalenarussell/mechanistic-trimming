calculate_subject_gene_weight <- function(compiled_data){
    ...
    subject_sum$gene_weight_type = GENE_WEIGHT_TYPE
    together = merge(compiled_data, subject_sum[, c('gene', 'p_gene', 'gene_weight_type')], by = 'gene')
    together$p_trim_given_gene = together$count/together$gene_count
    together$weighted_observation = together$p_trim_given_gene * together$p_gene
    return(together)
}
