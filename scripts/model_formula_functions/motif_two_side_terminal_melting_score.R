...
get_start_list <- function(motif_data){
    ...
    start_list = rep(0, positions*3 + 2)
    return(start_list)
}
...
get_model_formula <- function(){
    ...
    formula = formula(paste0('cbind(weighted_observation, interaction(gene, subject)) ~ ', motif_positions_together, ' + left_terminal_melting_score + right_terminal_melting_score'))
    return(formula)
}
