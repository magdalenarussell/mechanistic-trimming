...
get_parameter_vector <- function(trims, genes){
    ...
    mh_interaction = get_all_mh_prop_length_interaction_variables(overlap_vector = c(0, 1, 2, 3, 4))
    ...
    for (i in seq(length(trims))){
        ...
        right_vars = c(right_vars, get_all_base_prop_length_interaction_variables('3end', trims[i]))
    }

    return(c(motif_positions, left_vars, right_vars, mh_vars, mh_interaction, 'v_length', 'j_length'))
}
...
process_single_data_for_model_fit <- function(group_motif_data, whole_nucseq = get_oriented_whole_nucseqs(), gene_type = GENE_NAME, trim_type = TRIM_TYPE){
    ...
    down_mh_cols = colnames(together)[colnames(together) %like% 'mh_prop_down']
    up_mh_cols = colnames(together)[colnames(together) %like% 'mh_prop_up']
    
    for (var in down_mh_cols) {
        new_col = paste0(var, '_v_length_interaction')
        together[, paste(new_col) := get(var) * v_length]
    }

    for (var in up_mh_cols) {
        new_col = paste0(var, '_j_length_interaction')
        together[, paste(new_col) := get(var) * j_length]
    }

    base_count_prop_cols = colnames(together)[colnames(together) %like% '3end_base_count']

    for (var in base_count_prop_cols){
        if (var %like% 'v_trim'){
            new_col = paste0(var, '_v_length_interaction')
            together[, paste(new_col) := get(var) * v_length]
        } else if (var %like% 'j_trim'){
            new_col = paste0(var, '_j_length_interaction')
            together[, paste(new_col) := get(var) * j_length]
        }
    }
