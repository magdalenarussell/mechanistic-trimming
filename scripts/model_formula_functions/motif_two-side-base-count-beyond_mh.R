...
get_model_formula <- function(trim_type = TRIM_TYPE, gene_type = GENE_NAME){
    ...
    formula = formula(paste0('cbind(weighted_observation, interaction(', gene_groups, ', subject)) ~ ', left_vars_collapse, ' + ', right_vars_collapse, ' + ', motif_positions_together, ' + ', mh_vars_collapse))
