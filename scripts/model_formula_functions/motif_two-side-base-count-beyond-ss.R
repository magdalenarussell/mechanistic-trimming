...
get_model_formula <- function(){
    ...
    formula = formula(paste0('cbind(weighted_observation, interaction(gene, subject)) ~ ', left_vars_collapse, ' + ', right_vars_collapse, ' + ', motif_positions_together))

    return(formula)
}

process_data_for_model_fit <- function(group_motif_data){
    together = process_for_two_side_base_count(group_motif_data, beyond_motif = TRUE, single_stranded = TRUE)
    return(together)
}
