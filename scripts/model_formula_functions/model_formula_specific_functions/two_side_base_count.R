...
get_left_right_seq_vars <- function(motif_data, left_nuc_count = LEFT_SIDE_TERMINAL_MELT_LENGTH, beyond_motif, single_stranded = FALSE, whole_nucseq = get_oriented_whole_nucseqs(), gene_type = GENE_NAME, trim_type = TRIM_TYPE){
    ...
    genes = whole_nucseq$gene[substring(whole_nucseq$gene, 4, 4) == toupper(substring(gene_type, 1, 1))]
...
process_for_two_side_base_count <- function(motif_data, left_nuc_count = LEFT_SIDE_TERMINAL_MELT_LENGTH, beyond_motif = FALSE, single_stranded = FALSE, whole_nucseq = get_oriented_whole_nucseqs(), trim_type = TRIM_TYPE){
