...
get_model_formula <- function(){
    ...
    formula = formula(paste0('cbind(weighted_observation, interaction(gene, subject)) ~ ', motif_positions_together, ' + as.factor(trim_length) + left_terminal_melting_score + right_terminal_melting_score'))
    return(formula)
}
