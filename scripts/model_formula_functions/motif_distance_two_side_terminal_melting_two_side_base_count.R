...
get_model_formula <- function(){
    ...
    formula = formula(paste0('cbind(weighted_observation, interaction(gene, subject)) ~ ', motif_positions_together, ' + ', left_vars_collapse, ' + ', right_vars_collapse, ' + as.factor(trim_length) + left_terminal_melting + right_terminal_melting'))
    return(formula)
}
