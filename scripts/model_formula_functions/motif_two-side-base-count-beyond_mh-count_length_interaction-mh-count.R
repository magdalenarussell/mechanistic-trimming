...
get_parameter_vector <- function(trims, genes){
    ...
    mh_interaction = get_all_mh_count_length_interaction_variables(overlap_vector = c(0, 1, 2, 3, 4))

    left_vars = c()
    right_vars = c()
    for (i in seq(length(trims))){
        left_vars = c(left_vars, paste0(get_all_base_variables('5end', trims[i])))
        right_vars = c(right_vars, paste0(get_all_base_variables('3end', trims[i])))
    }
...
get_model_formula <- function(trim_type = TRIM_TYPE, gene_type = GENE_NAME){
    ...
    mh_vars = vars[vars %like% 'mh_count']
...
process_single_data_for_model_fit <- function(group_motif_data, whole_nucseq = get_oriented_whole_nucseqs(), gene_type = GENE_NAME, trim_type = TRIM_TYPE){
    ...
    down_mh_cols = colnames(together)[colnames(together) %like% 'mh_count_down']
    up_mh_cols = colnames(together)[colnames(together) %like% 'mh_count_up']
