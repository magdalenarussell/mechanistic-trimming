...
get_start_list <- function(motif_data){
    ...
    start_list = rep(0, unique_motifs-1 + 2)
    return(start_list)
}
...
get_model_formula <- function(){
    formula = formula(paste0('cbind(weighted_observation, interaction(gene, subject)) ~ as.factor(motif) + left_terminal_melting_score + right_terminal_melting_score'))
    return(formula)
}
