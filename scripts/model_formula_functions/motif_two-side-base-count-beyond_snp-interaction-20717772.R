...
get_model_formula <- function(trim_type = TRIM_TYPE, gene_type = GENE_NAME){
    ...
    interaction_term = paste(paste0('as.numeric(', trims, '))*snp'), collapse = ' + ')

    formula = formula(paste0('cbind(weighted_observation, interaction(', gene_groups,' ,subject)) ~ (', left_vars_collapse, ' + ', right_vars_collapse, ' + ', motif_positions_together, ' + ', interaction_term))

    return(formula)
}

process_single_data_for_model_fit <- function(group_motif_data, gene_type = GENE_NAME, trim_type = TRIM_TYPE){
    together = process_for_two_side_base_count(group_motif_data, beyond_motif = TRUE, gene_type = gene_type, trim_type = trim_type)
    snp = 20717772
