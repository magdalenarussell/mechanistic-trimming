...
get_model_formula <- function(){
    ...
    formula = formula(paste0('cbind(weighted_observation, interaction(gene, subject)) ~ ', left_vars_collapse, ' + ', right_vars_collapse, ' + left_terminal_melting + right_terminal_melting + as.factor(trim_length)'))
    return(formula)
}
