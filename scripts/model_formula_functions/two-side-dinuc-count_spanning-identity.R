...
get_model_formula <- function(){
    left_vars = get_all_dinucs_variables('left')
    ...
    formula = formula(paste0('cbind(weighted_observation, interaction(gene, subject)) ~ ', left_vars_collapse, ' + ', right_vars_collapse, ' + as.factor(span_identity)'))
    return(formula)
}

process_data_for_model_fit <- function(group_motif_data){
    together = process_for_two_side_dinuc_count(group_motif_data)
    return(together)
}
