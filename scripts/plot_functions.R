get_PFM <- function(group_name, trim_lengths_list){
    output_path = file.path('_ignore', 'pfms', 'complete') 
    file = file.path(output_path, paste0(group_name, '_', PFM_TYPE, '_', MOTIF_TYPE, '_', paste(trim_lengths_list, collapse = ''), '.tsv'))
    PFM = as.matrix(fread(file), rownames = 1)
    return(PFM)
}


create_plot_title <- function(group_name){
    if (group_name == 'ALL'){
...
create_file_name <- function(group_name, trim_lengths_list, plot_type){
    output_path = file.path('plots') 
    dir.create(output_path, recursive = TRUE, showWarnings = FALSE)
    filename = file.path(output_path, paste0(group_name, '_', PFM_TYPE, '_', MOTIF_TYPE, '_', paste(trim_lengths_list, collapse = ''), '_', plot_type, '.pdf'))
    return(filename)
}
...
plot_single_PWM_heatmap <- function(group_name, trim_lengths_list, subtitle = NULL){
    PFM = get_PFM(group_name, trim_lengths_list)
    plot_title = create_plot_title(group_name)
    filename = create_file_name(group_name, trim_lengths_list, 'PWM_heatmap_with_background_correction')
    
    PWM = calculate_PWM_from_PFM(PFM)
...
plot_PWM_heatmap_by_genotype <- function(snpID, trim_lengths_list, subtitle = NULL){
    together = data.frame()
    for (genotype in c(0,1,2)){
        assign(paste0('PFM', genotype), get_PFM(group_name = get_group_name(snpID, genotype) , trim_lengths_list))
        assign(paste0('PWM', genotype), calculate_PWM_from_PFM(get(paste0('PFM', genotype))))
        PWM_df = transform_PWM_to_dataframe(get(paste0('PWM', genotype)))
        PWM_df$genotype = genotype
        together = rbind(together, PWM_df)
    }

    plot_title = create_plot_title(group_name = paste(snpID))
    
    filename = create_file_name(group_name = paste(snpID), trim_lengths_list, 'PWM_heatmap_by_GENOTYPE_with_background_correction')
