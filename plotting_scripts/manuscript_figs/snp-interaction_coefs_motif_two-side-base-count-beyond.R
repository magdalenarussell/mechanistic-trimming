...
first_grid = plot_grid(all[[1]], all[[2]], nrow = 1, labels = c("A", "B"), label_size = 35, rel_widths = c(0.5, 1), align = 'h')

path = get_manuscript_path()
file_name = paste0(path, '/snp_interaction_heatmaps.pdf')
ggsave(file_name, plot = first_grid, width = 20, height = 6, units = 'in', dpi = 750, device = cairo_pdf)
