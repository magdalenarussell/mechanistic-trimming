...
for (param in LIGATION_PARAMS){
    for (trim_model in TRIMMING_PROB_MODELS){
        for (model_type in MODEL_TYPES){
            ...
            #TODO change later
            ANNOTATION_TYPE <<- paste0('old_experiments/', old_annotation, '_from_', trim_model, '_MHprob', param)
...
fwrite(subset_wide, paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/scatter_MH_signal_between_trim_params/all_coefs.tsv'), sep = '\t')

scatter = ggplot(subset_wide)+
    geom_abline(intercept = 0, slope = 1, size = 3, color = 'gray60', linetype = 'dashed') +
    geom_point(aes(x = get('motif_two-side-base-count-beyond'), y = get('igor'), color = ligation_param, shape = model_type), size = 10) +
    xlab('MH coefficient for trims sampled according to\nmotif + base-count-beyond probabilities')+
    ylab('MH coefficient for trims sampled according to\nIGoR probabilities')+
    theme_cowplot(font_family = 'Arial') + 
    labs(color = 'MH simulation parameter', shape = 'Model trained') +
...
# save plot
file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/scatter_MH_signal_between_trim_params/rel_MH_coefs.pdf')
