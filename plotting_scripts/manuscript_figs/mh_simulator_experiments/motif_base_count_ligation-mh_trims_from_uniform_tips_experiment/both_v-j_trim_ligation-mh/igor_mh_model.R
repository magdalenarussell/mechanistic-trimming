...
LIGATION_PARAMS <<- 0
TRIMMING_PROB_MODEL <<- 'uniform_tips'
...
ANNOTATION_TYPE <<- paste0(old_annotation, '_from_', TRIMMING_PROB_MODEL, '_MHprob', LIGATION_PARAMS)
# Read in model coefficient data 
coef_path = get_model_coef_file_path(L2)
if (!file.exists(coef_path)){
    assign(paste0('together', LIGATION_PARAMS), NULL)

    next
}
coefs = fread(coef_path)

fwrite(coefs, paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/motif_base_count_ligation-mh_trims_from_uniform_tips_experiment/', PARAM_GROUP, '/coefs_MHprob', LIGATION_PARAMS, '_', TRIMMING_PROB_MODEL, '_trim-model.tsv'), sep = '\t')
...
assign(paste0('together', LIGATION_PARAMS), plot_grid(first_grid, NULL, second_grid, NULL, third_grid, NULL, legend, nrow = 7, rel_heights = c(1, 0.08, 1, 0.08, 0.75, 0.08, 0.2)))

# save plot
file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/motif_base_count_ligation-mh_trims_from_uniform_tips_experiment/', PARAM_GROUP, '/coef_heatmap_', TRIMMING_PROB_MODEL, '_trim-model.pdf')

ggsave(file_name, plot = together0, width = 18, height = 21.5, units = 'in', dpi = 750, device = cairo_pdf, limitsize=FALSE)
