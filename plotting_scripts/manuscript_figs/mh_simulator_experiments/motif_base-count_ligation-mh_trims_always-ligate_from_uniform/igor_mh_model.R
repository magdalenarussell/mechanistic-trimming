...
for (trim_model in TRIMMING_PROB_MODEL){
    for (subtype in ANNOTATION_SUBTYPE){
        for (param in LIGATION_PARAMS){
            ...
            fwrite(coefs, paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/motif_base-count_ligation-mh_trims_always-ligate_from_uniform/coefs_MHprob', param, '_', subtype, '_', trim_model, '_trim-model.tsv'), sep = '\t')
        ...
        # save plot
        file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/motif_base-count_ligation-mh_trims_always-ligate_from_uniform/coef_heatmap_', subtype, '_', trim_model, '_trim-model.pdf')
