...
for (param in LIGATION_PARAMS){
    ...
    fwrite(coefs, paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/baseline_igor_ligation-mh_trims_from_motif_base-count_experiment/coefs_MHprob', param, '.tsv'), sep = '\t')

    v_igor_heatmap = plot_igor_coefficient_heatmap_single_group(coefs[coefficient %like% 'v_trim_prob'], with_values = FALSE, limits = c(-0.555, 0.555))+ ggtitle('           V-trimming')

    j_igor_heatmap = plot_igor_coefficient_heatmap_single_group(coefs[coefficient %like% 'j_trim_prob'], with_values = FALSE, limits = c(-0.555, 0.555)) + ggtitle('          J-trimming')

    v_igor_heatmap = v_igor_heatmap + theme(legend.position = 'none') 
    j_igor_heatmap = j_igor_heatmap + theme(legend.position = 'none') 

    # plot mh heatmap
    mh_heatmap = plot_ligation_mh_coefficient_heatmap_single_group(coefs, with_values = FALSE, limits = c(-0.555, 0.555))
    ...
    # save plot
    file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/baseline_igor_ligation-mh_trims_from_motif_base-count_experiment/coef_heatmap_MHprob', param, '.pdf')
