...
for (trim_model in TRIMMING_PROB_MODEL){
    for (param in LIGATION_PARAMS){
        ANNOTATION_TYPE <<- 'igor_adjusted-mh_sim_alpha'
        ...
        fwrite(coefs, paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/motif_base_count_ligation-mh_trims_from_motif_base-count_experiment_adjusted-mh/', PARAM_GROUP, '/coefs_MHprob', param, '_', trim_model, '_trim-model.tsv'), sep = '\t')

        v_motif_heatmap = plot_motif_coefficient_heatmap_single_group(coefs[trim_type == 'v_trim'], with_values = FALSE, limits = c(-0.700, 0.700)) + ggtitle('   V-trimming coefficients')
        j_motif_heatmap = plot_motif_coefficient_heatmap_single_group(coefs[trim_type == 'j_trim'], with_values = FALSE, limits = c(-0.700, 0.700)) + ggtitle('   J-trimming coefficients')
        ...
        v_base_count_heatmap = plot_base_count_coefficient_heatmap_single_group(coefs[trim_type == 'v_trim'], with_values = FALSE, limits = c(-0.700, 0.700))
        j_base_count_heatmap = plot_base_count_coefficient_heatmap_single_group(coefs[trim_type == 'j_trim'], with_values = FALSE, limits = c(-0.700, 0.700))
        ...
        # plot mh heatmap
        mh_heatmap = plot_ligation_mh_coefficient_heatmap_single_group(coefs, with_values = FALSE, limits = c(-0.700, 0.700))
    ...
    # save plot
    file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/motif_base_count_ligation-mh_trims_from_motif_base-count_experiment_adjusted-mh/', PARAM_GROUP, '/coef_heatmap_', trim_model, '_trim-model.pdf')
