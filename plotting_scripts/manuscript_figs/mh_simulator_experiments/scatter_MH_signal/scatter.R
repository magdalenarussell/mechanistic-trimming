...
cols = c('coefficient', 'ligation_param', 'trimming_prob_model', 'model_type', 'value')
subset_wide = subset[, ..cols] %>%
                pivot_wider(names_from = model_type, values_from = value)%>%
                as.data.table()

fwrite(subset_wide, paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/scatter_MH_signal/all_coefs.tsv'), sep = '\t')

#TODO remove after data available
temp = data.table(coefficient = 'ligation_mh', ligation_param = c(3.0, 3.0, 2.0, 2.0, 1.5, 1.5, 1.0, 1.0), trimming_prob_model = rep(c('igor', 'motif_two-side-base-count-beyond'), 4))
temp[['motif_two-side-base-count-beyond_ligation-mh']] = 1
subset_wide = merge(subset_wide, temp)

scatter = ggplot(subset_wide)+
    geom_abline(intercept = 0, slope = 1, size = 3, color = 'gray60', linetype = 'dashed') +
    geom_point(aes(x = get('motif_two-side-base-count-beyond_ligation-mh'), y = get('baseline_igor_norm_ligation-mh'), color = ligation_param, shape = trimming_prob_model), size = 10) +
    xlab('MH coefficient from motif + base-count-beyond + ligation MH model')+
    ylab('MH coefficient from IGoR + ligation MH model')+
    theme_cowplot(font_family = 'Arial') + 
    labs(color = 'MH simulation parameter', shape = 'Trimming probability model') +
...
# save plot
file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/mh_simulator_experiments/scatter_MH_signal/rel_MH_coefs.pdf')
