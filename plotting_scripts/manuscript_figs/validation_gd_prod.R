...
all_eval_results$productivity = 'nonproductive'
# annotation_types = c('validation_data_alpha', 'validation_data_beta', 'validation_data_gamma')
annotation_types = c('validation_data_alpha', 'validation_data_beta', 'validation_data_gamma', 'validation_data_delta', 'validation_data_igh', 'igor')
...
for (trim_type in trim_types){
    for (type in annotation_types) {
        for (prod in prods){
            ...
            GENE_NAME <<- paste0(substring(TRIM_TYPE, 1, 1), '_gene')
            PRODUCTIVITY <<- prod
            if (type %in% c('validation_data_gamma', 'validation_data_delta', 'igor') & prod == 'nonproductive'){
                next
            }
...
all_eval_results[loss_type == 'igor', loss_type := paste0(loss_type, '_', trim_type)]
