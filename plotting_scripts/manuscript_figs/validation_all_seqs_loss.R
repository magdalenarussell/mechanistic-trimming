...
TYPE <<- 'log_loss'
LOSS_GENE_WEIGHT <<- 'p_gene_given_subject_all_seqs' 
...
for (prod in c('productive', 'nonproductive')) {
    ...
    plot = ggplot(subset) +
        ...
        xlab(' ') +
        ylab('Expected per-sequence log loss (all sequence gene usage)\n')+ 
    ...
    file_name = paste0(path, '/loss_compare_validation_', prod, '_all_seqs_loss.pdf')
