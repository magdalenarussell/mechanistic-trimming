...
for (valid in c('igor', 'validation_data_igh', 'validation_data_delta', 'validation_data_gamma')){
    for (type in c('productive', 'nonproductive')){
        ...
        pwm_scores = get_all_pwm_score(data, pwm, rmse)
        cols = c('gene', 'trim_length', 'motif', 'pwm_score')
        data = merge(data, pwm_scores[, ..cols], by = c('gene', 'trim_length', 'motif'))

        # average across people
        total_subj = length(unique(data$subject))
        temp = data[, sum(weighted_observation), by = .(pwm_score, gene, type, validation_type, motif)]
        setnames(temp, 'V1', 'total_weighted_obs')
        seq_weights = rbind(seq_weights, temp)
        temp2 = data[, sum(weighted_log_lik), by = .(pwm_score, gene, type, validation_type, motif)]
        setnames(temp2, 'V1', 'weighted_log_lik')
        log_lik = rbind(log_lik, temp2)
        cols = c('log_pred', 'gene', 'pwm_score', 'type', 'validation_type', 'motif')
...
plot = ggplot(seq_weights, aes(x = pwm_score, y =total_weighted_obs)) +
...
ggsave(file.path(path, 'avg_weight_by_pwm_score.pdf'), plot = plot, width = 40, height = 15, units = 'in', dpi = 750, device = cairo_pdf)

plot2 = ggplot(log_lik, aes(x = pwm_score, y =weighted_log_lik)) +
    geom_point(alpha = 0.5, size = 3) +
    facet_grid(rows = vars(type), cols = vars(validation_type))+
    # scale_y_continuous(trans = scales::log_trans(), 
                       # breaks = scales::log_breaks())+
...
ggsave(file.path(path, 'avg_weighted_log_lik_by_pwm_score.pdf'), plot = plot2, width = 40, height = 15, units = 'in', dpi = 750, device = cairo_pdf)

plot3 = ggplot(model_prob, aes(x = pwm_score, y = log_pred)) +
    geom_point(alpha = 0.5, size = 3) +
    facet_grid(rows = vars(type), cols = vars(validation_type))+
    # scale_y_continuous(trans = scales::log_trans(), 
                       # breaks = scales::log_breaks())+
...
ggsave(file.path(path, 'pred_by_pwm_score.pdf'), plot = plot3, width = 40, height = 15, units = 'in', dpi = 750, device = cairo_pdf)
