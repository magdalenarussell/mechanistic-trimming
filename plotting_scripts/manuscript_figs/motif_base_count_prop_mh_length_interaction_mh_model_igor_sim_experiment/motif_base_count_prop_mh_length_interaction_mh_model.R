...
MODEL_TYPE <<- 'motif_two-side-base-count-beyond-prop_mh_length_interaction_mh'
...
v_motif_heatmap = plot_motif_coefficient_heatmap_single_group(coefs[trim_type == 'v_trim'], with_values = FALSE, limits = c(-0.316, 0.316)) + ggtitle('   V-trimming coefficients')
j_motif_heatmap = plot_motif_coefficient_heatmap_single_group(coefs[trim_type == 'j_trim'], with_values = FALSE, limits = c(-0.316, 0.316)) + ggtitle('   J-trimming coefficients')
...
# plot mh heatmap
interaction_heatmap = plot_mh_coefficient_heatmap_single_group(coefs, with_values = FALSE, limits = c(-0.316, 0.316), interaction_coefs = TRUE)
interaction_heatmap = interaction_heatmap + theme(legend.position = 'none')
...
all = align_plots(v_motif_heatmap, j_motif_heatmap, v_base_count_heatmap, j_base_count_heatmap, v_length_heatmap, j_length_heatmap, mh_heatmap, interaction_heatmap, legend, align = 'vh', axis = 'lbr')
...
fifth_grid = plot_grid(NULL, all[[8]], NULL, nrow = 1, rel_widths = c(0.1, 1, 0.1), align = 'h')

together = plot_grid(first_grid, NULL, second_grid, NULL, third_grid, NULL, fourth_grid, NULL, fifth_grid, NULL, legend, nrow = 11, rel_heights = c(1, 0.05, 1, 0.05, 1, 0.05, 1.5, 0.05, 1.5, 0.05, 0.2))

# save plot
file_name = paste0(MOD_PROJECT_PATH, '/plotting_scripts/manuscript_figs/motif_base_count_prop_mh_length_interaction_mh_model_igor_sim_experiment/coef_heatmap.pdf')

ggsave(file_name, plot = together, width = 18, height = 40, units = 'in', dpi = 750, device = cairo_pdf)
